...
import { TokenData, GlobalAuthConfig } from '../types';
import { Logger } from '../utils/logger';

/**
 * In-memory global variable authentication strategy
 */
export class GlobalAuthStrategy extends AuthStrategy {
  private static tokenStore: Map<string, string> = new Map();
  private instanceId: string;

  constructor(config: GlobalAuthConfig = {}, logger?: Logger) {
  ...
  async saveToken(tokenData: TokenData): Promise<void> {
    try {
      ...
      GlobalAuthStrategy.tokenStore.set(this.getStorageKey(), encrypted);
  ...
  async loadToken(): Promise<TokenData | null> {
    try {
      ...
      const encrypted = GlobalAuthStrategy.tokenStore.get(this.getStorageKey());
  ...
  async deleteToken(): Promise<void> {
    try {
      ...
      GlobalAuthStrategy.tokenStore.delete(this.getStorageKey());
  ...
  static clearAll(): void {
    GlobalAuthStrategy.tokenStore.clear();
  }
}
